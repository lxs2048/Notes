...
同时在使用命令行的时候可以传递一些options，如`npx lessc --math="always" test.less`，如下如果不使用参数margin是不会被计算的
...
然而目前less和sass等一系列预处理器的postcss插件已经停止维护了，维护成本较大，所以需要使用各自的编译工具编译完, 然后把编译结果给postcss，**所以业内就产生了一个新的说法: postcss是后处理器**
